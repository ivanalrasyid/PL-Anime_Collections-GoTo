...
export default function AnimeListPage() {
  ...
  const { loading, error, data } = useQuery<AnimeListData, AnimeListVars>(
    ANIME_LIST,
    {
      variables: {
        page,
  ...
  if (loading)
    return (
      <Loading wrapperCss={loadingPageStyle} message="Loading Anime List" />
    )
    ...
    <div id="animeListPage" css={AnimeListPageStyle}>
      <Header>
        <span css={logoContainerStyle}>
          {!isBulkChooseActive && <img src={logo} alt="Anime Club logo" />}
          {isBulkChooseActive ? (
            <span css={instructionStyle}>
              Select at least 1 Anime to proceed
            </span>
          ) : (
            'Anime Club'
          )}
        ...
        <span css={btnContainerStyle}>
          {isBulkChooseActive && (
            <Button text="Cancel" type="ghost" isLarge onClick={handleCancel} />
          )}
